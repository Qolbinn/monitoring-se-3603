...
export function ColumnChartCard({ currentDate, kodeKecamatan, kecamatanOptions, chartData, idealPercentage, lastUpdatedTime, surveyPeriod }: ColumnChartCardProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  ...
  return (
    <Card className="rounded-[16px] border border-border bg-card shadow-sm col-span-1 md:col-span-2">
      ...
      <CardContent id="column-chart-card" className="pt-6 bg-card rounded-b-[16px] p-4 sm:p-6 relative min-h-[300px]">
        {isPending && (
